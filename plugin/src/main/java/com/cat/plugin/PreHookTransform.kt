...
class PreHookTransform : Transform() {

    lateinit var hookItemsFile: File
    ...
    override fun transform(ti: TransformInvocation) {
        ...
        if (!hookItemsFile.exists()) {
            hookItemsFile.createNewFile()
        }
        hookItems.clear()
            ...
            it.directoryInputs.forEach { directoryInput ->
                ...
                directoryInput.file.walk().forEach { file ->
                    if (!file.isDirectory) {
                        ...
                        executor.execute {
                            doPreHookTransfer(classPath, file.readBytes())
                        }
                    }
                }
            }
        ...
        val fw = FileWriter(hookItemsFile)
        fw.write(Gson().toJson(hookItems))
        fw.append()
    ...
    private fun processPreHookJarFile(file: File) {
        val zipFile = ZipFile(file)
        zipFile.entries().toList().forEach {
            val name = it.name
            var bytes = zipFile.getInputStream(it).readBytes()
            if (name.endsWith(".class")) {
                doPreHookTransfer(name, bytes)
            }
        }
    }

    private fun doPreHookTransfer(classPath: String, input: ByteArray) {
        val s = String(input)
